...
class SequenceDecisionEnvironmentSB3(Environment):
    ...
    def step(self, action):
        total_rate, channal_power_set = self.cal_sumrate(action)

        reward = total_rate
        H_dB_new = channal_power_set.reshape(-1, )
        if self.cnt >= self.maxcnt:
            self._history_channel_information = H_dB_new
            self.cnt = 0
        else:
            # self._history_channel_information don't change
            self.cnt += 1

        obs = np.concatenate([self._history_channel_information, action.reshape(-1, )], axis=-1)
        terminated, truncated, info = False, False, {}
        return obs, reward, terminated, truncated, info

...
_version = 'seqPPOcons_burst10'
# burst10 means the user demand change every 10 time slot
episode_length = 50
_load_env = 1
_load_model = 0
...
if _load_model:
    tr_args['policy'] = SequenceActorCriticPolicy #todo
    model = SequencePPO.load(
        path=
'D:\PythonProject\RRM_ppo\Experiment_result\seqPPOcons_burst\BS1UE20\E1\date20250309time203115\model_saves\seqPPOcons_burst_NumSteps_251904.zip',        env=env,
        **tr_args,
    )
else:
...
# todo user_busrt of training_env and eval_env should be same.
...
# train the model
model.learn(total_timesteps=episode_length * 5000 * 5, progress_bar=True, log_interval=10,

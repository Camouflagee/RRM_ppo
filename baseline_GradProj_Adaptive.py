...
for idx, (nUE, nRB) in enumerate(
    ...
    test_num = 10
    for _error_percent in [0.01, 0.05, 0.1, 0.15]:
        print(f"场景: UE{nUE}RB{nRB} - _error_percent: {_error_percent}")
        ...
        for loop in range(test_num):
            ...
            error_percent = 0.05
            if isAdaptive:
                H_error = env.get_estimated_H(H, error_percent)  # add 5% estimated error
